...
func xunfeiStreamHandler(c *gin.Context, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string, info *relaycommon.RelayInfo) (*dto.Usage, *types.NewAPIError) {
...
func xunfeiHandler(c *gin.Context, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string, info *relaycommon.RelayInfo) (*dto.Usage, *types.NewAPIError) {
	domain, authUrl := getXunfeiAuthUrl(c, apiKey, apiSecret, textRequest.Model)
	dataChan, stopChan, err := xunfeiMakeRequest(textRequest, domain, authUrl, appId, info)
	if err != nil {
		return nil, types.NewError(err, types.ErrorCodeDoRequestFailed)
...
func xunfeiMakeRequest(textRequest dto.GeneralOpenAIRequest, domain, authUrl, appId string, info *relaycommon.RelayInfo) (chan XunfeiChatResponse, chan bool, error) {
	...
	conn, resp, err := d.Dial(authUrl, buildXunfeiDialHeader(info))
