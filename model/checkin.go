...
func UserCheckin(userId int) (*Checkin, error) {
	...
	// SQLite 单独走同语义实现，便于兼容该方言的事务行为
	if common.UsingSQLite {
		return userCheckinSQLite(checkin, userId, quotaAwarded)
	}
...
func userCheckinWithTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 步骤2: 在事务中增加用户账户余额
		if err := IncreaseUserAccountBalanceTx(tx, userId, quotaAwarded); err != nil {
...
// userCheckinSQLite 执行 SQLite 签到路径
func userCheckinSQLite(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	// 步骤1: 创建签到记录
	// 数据库有唯一约束 (user_id, checkin_date)，可以防止并发重复签到
