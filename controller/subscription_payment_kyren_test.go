...
func (f *kyrenCheckoutFakeAPI) listOrders(context.Context, string, string, int, int) (*kyrenOrderList, error) {
...
func TestSubscriptionKyrenRequiresExplicitPurchaseMode(t *testing.T) {
	setupKyrenPaymentControllerTestDB(t)
	userID := 6091
	seedKyrenPaymentUser(t, userID)
	plan := seedKyrenPaymentPlan(t, 6092, "prod_explicit_mode", 1000, 2)
	...
	var count int64
	require.NoError(t, model.DB.Model(&model.SubscriptionOrder{}).Where("user_id = ? AND plan_id = ?", userID, plan.Id).Count(&count).Error)
	assert.Zero(t, count)
}

func TestSubscriptionKyrenAllowsRenewalWhenHistoricalPurchaseLimitReached(t *testing.T) {
	setupKyrenPaymentControllerTestDB(t)
	userID := 6088
	seedKyrenPaymentUser(t, userID)
	plan := seedKyrenPaymentPlan(t, 6089, "prod_renewal", 1000, 2)
	require.NoError(t, model.DB.Model(&model.SubscriptionPlan{}).Where("id = ?", plan.Id).Update("max_purchase_per_user", 1).Error)
	require.NoError(t, model.DB.Create(&model.UserSubscription{UserId: userID, PlanId: plan.Id, Status: "active", StartTime: common.GetTimestamp() - 10, EndTime: common.GetTimestamp() + 3600, GrantReason: "order", Source: "order"}).Error)
	fake := &kyrenCheckoutFakeAPI{}
	withKyrenCheckoutFakeControllerClient(t, fake)
...
func TestSubscriptionKyrenCreditPurchasePersistsSnapshotBeforeCheckout(t *testing.T) {
	...
	userID := 6093
	seedKyrenPaymentUser(t, userID)
	seedExternalCreditPurchasePlans(t, 6094, 6095)
	require.NoError(t, model.DB.Model(&model.SubscriptionPlan{}).Where("id = ?", 6094).Update("kyren_product_id", "prod_credit_mode").Error)
	var observedOrder model.SubscriptionOrder
	var orderLookupErr error
	fake := &kyrenCheckoutFakeAPI{createCheckoutFunc: func(ctx context.Context, req kyrenCreateCheckoutRequest) (*kyrenCheckoutSession, error) {
		orderLookupErr = model.DB.Where("trade_no = ?", req.Metadata["trade_no"]).First(&observedOrder).Error
		return &kyrenCheckoutSession{ID: "chk_credit", URL: "https://pay.kyren.test/credit", Status: "open"}, nil
	}}
	withKyrenCheckoutFakeControllerClient(t, fake)

	recorder := performKyrenSubscriptionPayRequest(t, userID, `{"plan_id":6094,"purchase_mode":"credit_balance"}`)
	...
	assert.Equal(t, 6095, snapshot.TargetCreditBalancePlanID)
	assert.Equal(t, model.PaymentProviderKyren, snapshot.PaymentProvider)
	assert.Equal(t, "prod_credit_mode", snapshot.ProviderProductID)
	assert.Equal(t, model.PaymentMethodKyren, snapshot.ProviderPaymentMethod)
	assert.Equal(t, int64(4000), snapshot.PaymentAmountCents)
	assert.Equal(t, kyrenCurrencyCNY, snapshot.PaymentCurrency)
...
func TestSubscriptionKyrenCreditWebhookCompletesFromSnapshotWithoutInvitation(t *testing.T) {
	...
	fake := &kyrenCheckoutFakeAPI{}
	withKyrenCheckoutFakeControllerClient(t, fake)
	create := performKyrenSubscriptionPayRequest(t, userID, `{"plan_id":6097,"purchase_mode":"credit_balance"}`)
	...
	assert.Equal(t, ledger.UserSubscriptionId, cachedBuyer.GetSetting().ActiveSubscriptionId)
	var timedCount int64
	require.NoError(t, model.DB.Model(&model.UserSubscription{}).Where("user_id = ? AND plan_id = ?", userID, 6097).Count(&timedCount).Error)
...
func TestKyrenWebhookReturnsRetryableStatusWhileProviderEventClaimed(t *testing.T) {
	setupKyrenPaymentControllerTestDB(t)
	userID := 6132
	seedKyrenPaymentUser(t, userID)
	...
	require.NoError(t, model.DB.Create(&model.PaymentProviderEvent{
		Provider: model.PaymentProviderKyren, EventID: "evt_kyren_sub_claimed_retry", EventType: "order.paid",
		PayloadHash: hex.EncodeToString(payloadHash[:]), TradeNo: tradeNo, OrderKind: model.PaymentOrderKindSubscription,
		ProviderOrderID: "ord_kyren_sub_claimed_retry", Status: model.PaymentProviderEventProcessing,
		ProcessingToken: "active-event-token", ProcessingStartedAt: now, CreatedAt: now, UpdatedAt: now,
	}).Error)
...
func TestKyrenWebhookRecoversStaleProviderEventClaim(t *testing.T) {
	setupKyrenPaymentControllerTestDB(t)
	userID := 6142
	seedKyrenPaymentUser(t, userID)
	...
	now := common.GetTimestamp()
	require.NoError(t, model.DB.Create(&model.PaymentProviderEvent{
		Provider: model.PaymentProviderKyren, EventID: "evt_kyren_sub_stale_claim", EventType: "order.paid",
		PayloadHash: hex.EncodeToString(payloadHash[:]), TradeNo: tradeNo, OrderKind: model.PaymentOrderKindSubscription,
		ProviderOrderID: "ord_kyren_sub_stale_claim", Status: model.PaymentProviderEventProcessing,
		ProcessingToken: "stale-event-token", ProcessingStartedAt: now - 600, CreatedAt: now - 600, UpdatedAt: now - 600,
	}).Error)
	...
	assert.Equal(t, common.TopUpStatusSuccess, order.Status)
	var event model.PaymentProviderEvent
	require.NoError(t, model.DB.Where("provider = ? AND event_id = ?", model.PaymentProviderKyren, "evt_kyren_sub_stale_claim").First(&event).Error)
	assert.Equal(t, model.PaymentProviderEventApplied, event.Status)
...
func TestKyrenSuccessfulSubscriptionReplayStillValidatesPaymentSnapshot(t *testing.T) {
	setupKyrenPaymentControllerTestDB(t)
	userID := 6149
	seedKyrenPaymentUser(t, userID)
	plan := seedKyrenPaymentPlan(t, 6150, "prod_sub_success_replay", 1000, 1)
	tradeNo := "kyren-sub-success-replay"
	seedPendingKyrenSubscriptionOrder(t, tradeNo, userID, &plan)
	calledOrderIDs := make([]int, 0, 2)
	SetInvitationRewardOrderHandlerForTest(t, func(orderID int) error {
		calledOrderIDs = append(calledOrderIDs, orderID)
	...
	validPayload := kyrenWebhookEventPayload(t, "order.paid", "subscription", tradeNo, plan.KyrenProductId, "40.00", kyrenCurrencyCNY)
	require.Equal(t, http.StatusOK, performSignedKyrenWebhook(t, validPayload).Code)
	order := model.GetSubscriptionOrderByTradeNo(tradeNo)
	require.NotNil(t, order)
	require.Equal(t, []int{order.Id}, calledOrderIDs)

	mismatchedPayload := kyrenWebhookEventPayload(t, "order.paid", "subscription", tradeNo, plan.KyrenProductId, "41.00", kyrenCurrencyCNY)
	recorder := performSignedKyrenWebhook(t, mismatchedPayload)
...
func TestKyrenWebhookFailedFinalizesPendingOrdersWithoutOverwritingSuccess(t *testing.T) {
	setupKyrenPaymentControllerTestDB(t)
	userID := 6191
	seedKyrenPaymentUser(t, userID)
	plan := seedKyrenPaymentPlan(t, 6192, "prod_sub_failed", 1000, 1)
	subPending := "kyren-sub-failed-pending"
	subSuccess := "kyren-sub-failed-success"
	topUpPending := "kyren-topup-failed-pending"
	topUpSuccess := "kyren-topup-failed-success"
	seedPendingKyrenSubscriptionOrder(t, subPending, userID, &plan)
	seedPendingKyrenSubscriptionOrder(t, subSuccess, userID, &plan)
	require.NoError(t, model.DB.Model(&model.SubscriptionOrder{}).Where("trade_no = ?", subSuccess).Updates(map[string]any{"status": common.TopUpStatusSuccess, "complete_time": common.GetTimestamp()}).Error)
	seedPendingKyrenTopUp(t, topUpPending, userID, "topup_failed_pending", "prod_topup_failed", "10.00", 5000000)
	seedPendingKyrenTopUp(t, topUpSuccess, userID, "topup_failed_success", "prod_topup_failed", "10.00", 5000000)
	require.NoError(t, model.DB.Model(&model.TopUp{}).Where("trade_no = ?", topUpSuccess).Updates(map[string]any{"status": common.TopUpStatusSuccess, "complete_time": common.GetTimestamp()}).Error)

	for _, test := range []struct {
		kind, tradeNo, productID, amount string
	}{
		{kind: "subscription", tradeNo: subPending, productID: plan.KyrenProductId, amount: "40.00"},
		{kind: "subscription", tradeNo: subSuccess, productID: plan.KyrenProductId, amount: "40.00"},
		{kind: "topup", tradeNo: topUpPending, productID: "prod_topup_failed", amount: "10.00"},
		{kind: "topup", tradeNo: topUpSuccess, productID: "prod_topup_failed", amount: "10.00"},
	} {
		recorder := performSignedKyrenWebhook(t, kyrenWebhookEventPayload(t, "order.failed", test.kind, test.tradeNo, test.productID, test.amount, kyrenCurrencyCNY))
		require.Equal(t, http.StatusOK, recorder.Code, recorder.Body.String())
	}

	require.Equal(t, common.TopUpStatusFailed, model.GetSubscriptionOrderByTradeNo(subPending).Status)
	require.Equal(t, common.TopUpStatusSuccess, model.GetSubscriptionOrderByTradeNo(subSuccess).Status)
	require.Equal(t, common.TopUpStatusFailed, model.GetTopUpByTradeNo(topUpPending).Status)
	require.Equal(t, common.TopUpStatusSuccess, model.GetTopUpByTradeNo(topUpSuccess).Status)
	...
	require.NoError(t, model.DB.Where("event_type = ?", "order.failed").Order("event_id").Find(&events).Error)
	require.Len(t, events, 4)
	for _, event := range events {
		assert.Empty(t, event.ProcessingToken)
		if strings.Contains(event.TradeNo, "-pending") {
			assert.Equal(t, model.PaymentProviderEventApplied, event.Status)
		} else {
			assert.Equal(t, model.PaymentProviderEventConflict, event.Status)
		}
...
func TestKyrenWebhookRefundedRecordsManualActionAndReturnsSuccess(t *testing.T) {
	...
	require.NoError(t, model.DB.Transaction(func(tx *gorm.DB) error {
		...
		_, err := model.CompleteSubscriptionOrderTx(tx, &order, `{}`, model.PaymentMethodKyren)
		return err
	}))
...
func TestKyrenCheckoutRejectsArchivedOrMismatchedProductBeforeOrderCreation(t *testing.T) {
	...
	archivedRecorder := performKyrenSubscriptionPayRequest(t, userID, fmt.Sprintf(`{"plan_id":%d,"purchase_mode":"timed"}`, archivedPlan.Id))
	...
	mismatchRecorder := performKyrenSubscriptionPayRequest(t, userID, fmt.Sprintf(`{"plan_id":%d,"purchase_mode":"timed"}`, mismatchPlan.Id))
...
func TestKyrenCheckoutMetadataIncludesStableBusinessFields(t *testing.T) {
	...
	response := performKyrenSubscriptionPayRequest(t, userID, fmt.Sprintf(`{"plan_id":%d,"purchase_mode":"timed"}`, plan.Id))
...
func TestKyrenPayRejectsMissingWebhookSecret(t *testing.T) {
	...
	subRecorder := performKyrenSubscriptionPayRequest(t, userID, fmt.Sprintf(`{"plan_id":%d,"purchase_mode":"timed"}`, plan.Id))
