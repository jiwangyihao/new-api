...
type OAuthOnboardingProps = {
  pendingToken: string
  redirect?: string
  onComplete: (target: string) => void
}

function setAuthenticatedUser(userData: AuthUser) {
...
export function OAuthOnboarding(props: OAuthOnboardingProps) {
  ...
  useEffect(() => {
    ...
    api
      .get('/api/oauth/onboarding', {
        params: { pending_token: props.pendingToken },
  ...
  async function finishOnboarding() {
    ...
    if (requiresEmail && !verificationCode.trim()) {
      toast.error(t('Please enter the verification code'))
      return
    }
    const shouldValidatePassword = password !== '' || confirmPassword !== ''
    if (
    ...
    try {
      const res = await api.post('/api/oauth/onboarding', {
        pending_token: props.pendingToken,
      ...
      if (res.data?.success) {
        ...
        if (userData) {
          ...
          if (self?.success && self.data) {
            setAuthenticatedUser(self.data as AuthUser)
          }
        }
        toast.success(t('Account created successfully'))
        props.onComplete(props.redirect || '/dashboard')
      }
    } finally {
      setIsLoading(false)
    }
    ...
    <AuthLayout>
      <Card className='w-full'>
        ...
        <CardContent className='space-y-4'>
          ...
          <div className='grid gap-2'>
            <Label htmlFor='oauth-onboarding-confirm-password'>
              {t('Confirm password')}
            </Label>
            <PasswordInput
              id='oauth-onboarding-confirm-password'
