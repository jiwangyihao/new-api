...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if info.Billing == nil {
		if info.PriceData.FreeModel {
			logger.LogInfo(c, fmt.Sprintf("模型 %s 免费，仍执行订阅校验", info.OriginModelName))
			return nil, service.TaskErrorFromAPIError(types.NewOpenAIError(fmt.Errorf("非文本异步任务不支持分销订阅扣费"), types.ErrorCodeSubscriptionRequired, http.StatusForbidden, types.ErrOptionWithSkipRetry(), types.ErrOptionWithNoRecordErrorLog()))
		}
	...
	if adjustedRatios := adaptor.AdjustBillingOnSubmit(info, taskData); len(adjustedRatios) > 0 {
		...
		info.PriceData.OtherRatios = make(map[string]float64, len(adjustedRatios))
