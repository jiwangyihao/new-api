...
func TestAdminAnalyticsIgnoresBusinessGroupFiltersAndOmitsGroupDistributions(t *testing.T) {
	...
	query := AdminAnalyticsQuery{SnapshotAt: now, StartTimestamp: now - 120, EndTimestamp: now, Limit: 20}
...
func TestAdminUsageRejectsBusinessGroupByAndIgnoresRequestGroupFilter(t *testing.T) {
	...
	base := AdminAnalyticsQuery{StartTimestamp: now - 60, EndTimestamp: now, SnapshotAt: now, Limit: 20}
