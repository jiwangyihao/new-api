...
func TestExternalCreditPurchaseWebhookAndRefundLifecycle(t *testing.T) {
	...
	*commissionSetting = operation_setting.InvitationCommissionSetting{
	...
	require.True(t, ok)
	assert.Equal(t, "gpt-4o", creditPlanState["model_limits"])
	...
	allowsGlobalModel, err := model.HasActiveUserSubscriptionForModel(buyerID, "gpt-4o")
	require.NoError(t, err)
	assert.True(t, allowsGlobalModel)
	allowsOptionModel, err := model.HasActiveUserSubscriptionForModel(buyerID, "gpt-4o-mini")
	require.NoError(t, err)
	assert.False(t, allowsOptionModel, "model scope must come from the global Credit plan, not the Standard purchase option")
