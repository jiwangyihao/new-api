...
- 失败原子性 RED：DELETE 后注入错误时事务回滚，但旧返回值错误报告删除 2 条。GREEN：事务任何错误均返回 `(0, err)`；用例确认两条候选均保留。单用例 `count=1` 与全部 cleanup 用例 `count=10` 均为 `go test: 1 packages ok`，`git diff --check` 无输出。
