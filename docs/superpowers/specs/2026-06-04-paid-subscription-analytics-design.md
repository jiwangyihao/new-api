...
> **后续规格覆盖说明（2026-08-02）：** 本文关于 `SubscriptionPlan.price_amount > 0`、查询时读取当前套餐价格、以及 `min(token_based_value, time_based_value)` 统一适用于所有权益的假设，仅继续适用于尚未迁移到权益估值快照的计时套餐兼容路径。Credit 余额和计时权益价格快照的最终口径由 `2026-08-02-credit-operational-remaining-value-spec.md` 覆盖；不得再用本文第 5.1、5.4、7.3—7.7 节推导 Credit 运营剩余价值。
