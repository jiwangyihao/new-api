...
func TestSubscriptionPlansChannelTokenEquivalentsSingleAndRange(t *testing.T) {
	...
	assert.Equal(t, "single", openai.Kind)
	...
	assert.Equal(t, "single", gemini.Kind)
	...
	assert.Equal(t, "single", anthropic.Kind)
	...
	assert.Equal(t, "range", azure.Kind)
...
func TestChannelTokenMultiplierEndToEndSubscriptionPlansReflectCurrentChannelMultiplier(t *testing.T) {
	...
	assert.Equal(t, "single", updatedGemini.Kind)
...
func TestSubscriptionSelfSummaryChannelTokenEquivalentsIncludesZeroRemaining(t *testing.T) {
	...
	value, ok := summary["channel_token_equivalents"].([]interface{})
	require.True(t, ok, "channel_token_equivalents must be present as array")
	require.Len(t, value, 1)
	equivalent, ok := value[0].(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, "single", equivalent["kind"])
	assert.Equal(t, float64(constant.ChannelTypeOpenAI), equivalent["channel_type"])
	assert.Equal(t, float64(500_000), equivalent["equivalent_token_limit"])
	assert.Equal(t, float64(0), equivalent["equivalent_token_remaining"])
}
