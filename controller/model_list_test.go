...
func TestGetAllUsersReportsInviteCountsFromRelationships(t *testing.T) {
	...
	require.NoError(t, model.DB.Create(&model.User{Id: 9804, Username: "list-invitee-c", Status: common.UserStatusEnabled, AffCode: "list-c", InviterId: 9801}).Error)
	require.NoError(t, model.DB.Create(&model.User{Id: 9805, Username: "list-invitee-d", Status: common.UserStatusEnabled, AffCode: "list-d", InviterId: 9801}).Error)
	...
	require.NoError(t, model.DB.Create(&model.SubscriptionPlan{Id: 9813, Title: "Invite Trial", Enabled: true, DurationUnit: model.SubscriptionDurationMonth, DurationValue: 1, MonthlyTokenLimit: 1000, ConcurrencyLimit: 1, RewardEligible: true, InviteTrial: true}).Error)
	...
	require.NoError(t, model.DB.Create(&model.UserSubscription{Id: 9822, UserId: 9803, PlanId: 9812, Status: "active", StartTime: now - 60, EndTime: now + 3600, GrantReason: "redemption", Source: "redemption"}).Error)
	require.NoError(t, model.DB.Create(&model.UserSubscription{Id: 9823, UserId: 9804, PlanId: 9812, Status: "active", StartTime: now - 60, EndTime: now + 3600, GrantReason: model.SubscriptionGrantMonthlyInviteEntitlement, Source: model.SubscriptionGrantMonthlyInviteEntitlement}).Error)
	require.NoError(t, model.DB.Create(&model.UserSubscription{Id: 9824, UserId: 9801, PlanId: 9811, Status: "active", StartTime: now - 30, EndTime: now + 7200, GrantReason: model.SubscriptionGrantMonthlyInviteEntitlement, Source: model.SubscriptionGrantMonthlyInviteEntitlement}).Error)
	require.NoError(t, model.DB.Create(&model.UserSubscription{Id: 9825, UserId: 9805, PlanId: 9813, Status: "active", StartTime: now - 60, EndTime: now + 3600, GrantReason: model.SubscriptionGrantOrder, Source: model.SubscriptionGrantOrder}).Error)
	...
	assert.Equal(t, 4, inviter.DirectInviteCount)
	assert.Equal(t, 2, inviter.QualifiedPaidInviteCount)
