...
默认主题的侧边栏提供“渠道组可用性”页面（`/availability`），所有已登录用户均可访问。页面仅统计真实请求，不额外发送测活请求；渠道组遵循订阅页的筛选及归属规则，默认组不单列，也不因所有倍率为 1 而隐藏。

- 展示组及模型的近期状态、请求成功率、首响应延迟中位数和历史时间条；默认最近 24 小时，可切换 1 小时、7 天、30 天。页面可见时每 60 秒刷新，支持手动刷新。
- 同一次请求仅按最终结果计一次，重试成功不会重复计失败；明确的用户条件问题和客户端单独中断不进入分母。流式与异步任务按最终完成情况统计，接受任务不等于完成生成。
- 最近 15 分钟无有效请求显示未知；成功率 ≥99% 显示深绿，90%–<99% 显示浅绿，80%–<90% 显示黄色，50%–<80% 显示浅红，<50% 显示深红。样本较少、记录覆盖不足会单独提示，成功率不是按时间计算的在线率或 SLA 保证。
- 首次启用前缺少可靠归属快照的历史不会被猜测补齐；移动渠道不改写已记录的历史归属。页面不公开底层渠道身份、原始日志或精确请求计数。
- 观测记录与分钟汇总使用日志数据库，支持独立 `LOG_SQL_DSN`。自动迁移应由主节点执行；部署后须同步升级全部处理请求的节点，避免混合版本遗漏请求。汇总保留 31 天，请求去重身份保留 62 天。
