...
describe('api key user-facing configuration form', () => {
  ...
  test('defaults to no token limit and validates enabled token limits', () => {
    ...
    if (!zeroLimit.success) {
      assert.equal(zeroLimit.error.issues[0]?.message, 'Token limit must be greater than 0')
    }
    ...
    if (!decimalLimit.success) {
      assert.equal(decimalLimit.error.issues[0]?.message, 'Token limit must be greater than 0')
    }
    ...
    if (!nullLimit.success) {
      assert.equal(nullLimit.error.issues[0]?.message, 'Token limit must be greater than 0')
    }
    ...
    if (!nanLimit.success) {
      assert.equal(nanLimit.error.issues[0]?.message, 'Token limit must be greater than 0')
    }
    assert.equal(
      apiKeyFormSchema.safeParse({
        ...API_KEY_FORM_DEFAULT_VALUES,
